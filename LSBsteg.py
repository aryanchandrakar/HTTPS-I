...
def command(cmd,str):
    if cmd=="encode":
        ...
        formats = ["jpeg", "jpg"]
        data = str
        res = steg.encode_binary(data) # res -> pixel matrix
        cv2.imwrite(out_f, res)
        # return "Hidden"

    elif cmd=="decode":
        in_img = cv2.imread(str)
        steg = LSBSteg(in_img)
...
def convert_decode_byte2img(byts):
    img = Image.frombytes("RGB", (16, 16), byts)
    pixels = list(img.getdata())
    image_out = Image.new("RGB", (16, 16))
    image_out.putdata(pixels)
    imgsaved='server.png'
    image_out.save('server.png')
